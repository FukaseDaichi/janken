...
- `src/assets.ts` — 画像のロード。ファイル名は `IMAGE_FILES` に集約。読み込み失敗時は `Assets.get()` が undefined を返し、呼び出し側が代替描画にフォールバックする。スキン15枚とヒーロー画像は WebP（書き出しは `tools/make-skins.py` / `tools/make-hero.py`、要 Pillow）、デフォルト自機と敵は PNG。
