...
- タイトル画面の操作説明カード(`src/scenes/title.ts` の MOVE/DANGER/BREAK/LEVEL UP)への星の追記。
  無敵中は `DANGER: 弾・あいこ・負けの手は即死` が正しくなくなるが、星はマリオの慣習で
  説明なしに伝わり、取得時のフィードバック(黄フラッシュ・リング・BGM切替)も十分強いため、
  テキストでの追記は見送る
